...
class Anthropometry:
    def __init__(self, model):
        base_mesh_vertex_indices = (
            model.base_mesh_vertex_indices.detach().cpu().numpy().tolist()
        )
        self.model = model
        self.faces = model.faces
        if self.faces.shape[1] != 3:
            raise ValueError(
                "Faces must be triangles (3 vertices per face). Use a triangulated Anny model."
            )
        try:
            self.waist_vertex_indices = [
                base_mesh_vertex_indices.index(i) for i in BASE_MESH_WAIST_VERTICES
            ]
        except ValueError:
            raise ValueError(
                "Base mesh vertex indices do not contain all waist vertices."
            )

    def height(self, rest_vertices):
        return (
            torch.max(rest_vertices[..., 2], dim=1)[0]
            - torch.min(rest_vertices[..., 2], dim=1)[0]
        )
    ...
    def waist_circumference(self, rest_vertices):
        ...
        waist_circumference = torch.sum(
            torch.linalg.norm(waist_vertices_rolled - waist_vertices, dim=-1), dim=-1
        )
