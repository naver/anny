...
def get_face_segmentation_mask(anny_model, labels, image_path=None, metadata_path=None):
    if image_path is None:
        image_path = (
            get_anny_root_dir() / "data/segmentation/body_parts_segmentation.png"
        )

    if metadata_path is None:
        metadata_path = (
            get_anny_root_dir() / "data/segmentation/body_parts_segmentation.yaml"
        )
    ...
    u = (
        torch.round(
            face_center_texture_coordinates[:, 0]
            * body_parts_segmentation_array.shape[1]
        )
