...
def _shortest_arc_rotation(target: torch.Tensor, source: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    ...
    target_u = target / torch.clamp(torch.linalg.norm(target, dim=-1, keepdim=True), min=eps)
    source_u = source / torch.clamp(torch.linalg.norm(source, dim=-1, keepdim=True), min=eps)
    ...
    if torch.any(antiparallel_mask):
        ...
        R_180 = 2.0 * axis[..., :, None] * axis[..., None, :] - torch.eye(3, dtype=dtype, device=device)
        R = R.clone()
...
class ChildOffsetOrientationRefiner(torch.nn.Module):
    ...
    def __init__(
        ...
        self.bone_children_indices = make_buffer(self, "bone_children_indices", bone_children_indices, persistent=False)
        self.bone_children_mask = make_buffer(self, "bone_children_mask", bone_children_mask, persistent=False)
        self.bone_children_local_offsets = make_buffer(
            self, "bone_children_local_offsets", bone_children_local_offsets, persistent=False)

        # Static index groups. The root is never refined.
        counts = bone_children_mask.sum(dim=-1)
        counts[0] = 0
        leaf_bones = [bone_idx for bone_idx in range(1, bone_count) if bone_idx not in bone_parents]
        self.refined_single_child_bone_indices = make_buffer(
            self, "refined_single_child_bone_indices", torch.nonzero(counts == 1).squeeze(1), persistent=False)
        self.refined_multi_child_bone_indices = make_buffer(
            self, "refined_multi_child_bone_indices", torch.nonzero(counts >= 2).squeeze(1), persistent=False)
        self.leaf_bone_indices = make_buffer(
            self, "leaf_bone_indices", torch.tensor(leaf_bones, dtype=torch.int64), persistent=False)
        self.leaf_bone_parent_indices = make_buffer(
            self, "leaf_bone_parent_indices",
            torch.tensor([bone_parents[bone_idx] for bone_idx in leaf_bones], dtype=torch.int64),
            persistent=False)

    def forward(self, rest_bone_orientation: torch.Tensor, rest_bone_heads: torch.Tensor) -> torch.Tensor:
        # Offsets towards children bones: current ones, and bind-pose ones rotated by the current orientations.
        target_offsets = rest_bone_heads[:, self.bone_children_indices] - rest_bone_heads[:, :, None, :]
        target_offsets = target_offsets * self.bone_children_mask[None, :, :, None]
        source_offsets = torch.einsum("bkij,kcj->bkci", rest_bone_orientation, self.bone_children_local_offsets)
        ...
        if idx.numel() > 0:
            ...
            v_target = n_target * torch.linalg.norm(A[..., 0, :], dim=-1, keepdim=True) / (len_n_target + eps)
            v_source = n_source * torch.linalg.norm(B[..., 0, :], dim=-1, keepdim=True) / (len_n_source + eps)
            valid_normal = ((len_n_target[..., 0] > 1e-9) & (len_n_source[..., 0] > 1e-9))[..., None, None]
            H = H + torch.where(valid_normal, torch.einsum("bki,bkj->bkij", v_target, v_source), torch.zeros_like(H))
        ...
        if idx.numel() > 0:
            align = _shortest_arc_rotation(target_offsets[:, idx, 0], source_offsets[:, idx, 0])
            rest_bone_orientation[:, idx] = align @ rest_bone_orientation[:, idx]

        # End bones copy their parent's refined orientation.
        rest_bone_orientation[:, self.leaf_bone_indices] = rest_bone_orientation[:, self.leaf_bone_parent_indices]
