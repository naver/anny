...
def transfer_pose_parameters(
    src_model, src_pose_parameters, phenotype_kwargs, local_changes_kwargs, target_model
):
    ...
    missing = [
        label
        for label in target_model.bone_labels
        if label not in src_model.bone_labels
    ]
    assert not missing, (
        f"target_model has bones absent from src_model, cannot transfer the pose: {missing}"
    )
    bone_indices = [
        src_model.bone_labels.index(label) for label in target_model.bone_labels
    ]
    ...
    src_rest_inverse = (
        roma.Rigid.from_homogeneous(src_output["rest_bone_poses"][:, bone_indices])
        .inverse()
        .to_homogeneous()
    )
    target_bone_poses = (
        src_output["bone_poses"][:, bone_indices]
        @ src_rest_inverse
        @ base_output["rest_bone_poses"]
    )

    input_dict = dict(
        bone_poses=target_bone_poses, rest_bone_poses=base_output["rest_bone_poses"]
    )
    target_pose_parameters = target_model.get_pose_parameterization(
        input_dict, pose_parameterization=target_model.pose_parameterization
    )
