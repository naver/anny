...
@wp.func
def point_segment_projection(point: wp.vec3f, v0: wp.vec3f, v1: wp.vec3f) -> wp.vec3f:
    """
...
def get_point_triangle_distance_func(safe_length: bool = False):
    ...
    def point_triangle_distance(
        point: wp.vec3f, v0: wp.vec3f, v1: wp.vec3f, v2: wp.vec3f
    ) -> wp.float32:
...
def get_point_to_mesh_distance_kernel(safe_length: bool = True):
    ...
    try:
        ...
        @wp.kernel
        def point_to_mesh_distance_kernel(
            mesh_id: wp.uint64,
        ...
        _point_to_mesh_distance_kernel_cache[safe_length] = (
            point_to_mesh_distance_kernel
        )
...
def get_point_to_mesh_distance_and_face_kernel(safe_length: bool = False):
    try:
        ...
        @wp.kernel
        def point_to_mesh_distance_and_face_kernel(
            mesh_id: wp.uint64,
            points: wp.array(dtype=wp.vec3f),
            max_dist: wp.float32,
            distances: wp.array(dtype=wp.float32),
        ...
        _point_to_mesh_distance_and_face_kernel_cache[safe_length] = (
            point_to_mesh_distance_and_face_kernel
        )
...
def get_point_to_mesh_distance_and_face_uvs_kernel(safe_length: bool = False):
    try:
        ...
        @wp.kernel
        def point_to_mesh_distance_and_face_uvs_kernel(
            mesh_id: wp.uint64,
            points: wp.array(dtype=wp.vec3f),
            max_dist: wp.float32,
            distances: wp.array(dtype=wp.float32),
            ...
            if query.result:
                ...
                uvs[tid] = wp.vec2f(
                    query.u, query.v
                )  # Store the UV coordinates of the closest point
            else:
                distances[tid] = max_dist
                faces[tid] = -1  # No face found, set to -1 or any invalid index
                uvs[tid] = wp.vec2f(0.0, 0.0)  # No UV coordinates available

        _point_to_mesh_distance_and_face_uvs_kernel_cache[safe_length] = (
            point_to_mesh_distance_and_face_uvs_kernel
        )
...
class PointToMeshDistance(torch.autograd.Function):
    ...
    def forward(ctx, points, vertices, faces, max_dist, safe_length: bool = False):
        ...
        ctx.points = wp.from_torch(
            points.detach(), dtype=wp.vec3f, requires_grad=points.requires_grad
        )
        ctx.vertices = wp.from_torch(
            vertices.detach(), dtype=wp.vec3f, requires_grad=vertices.requires_grad
        )
        ctx.output = wp.zeros(
            ctx.dim,
            dtype=wp.float32,
            device=wp.device_from_torch(points.device),
            requires_grad=True,
        )
        ctx.mesh = wp.Mesh(
            points=ctx.vertices,
    ...
    def backward(ctx, grad_output):
        ...
        return (
            to_torch_or_none(ctx.points.grad),
            to_torch_or_none(ctx.vertices.grad),
            None,
            None,
            None,
        )
...
def point_to_mesh_distance(
    points, vertices, faces, max_dist, safe_length: bool = False
):
    return PointToMeshDistance.apply(
        points.contiguous(), vertices.contiguous(), faces, max_dist, safe_length
    )
...
class PointToMeshDistanceAndFace(torch.autograd.Function):
    ...
    def forward(ctx, points, vertices, faces, max_dist, safe_length: bool = False):
        ...
        ctx.distances_output = wp.zeros(
            ctx.dim,
            dtype=wp.float32,
            device=wp.device_from_torch(points.device),
            requires_grad=True,
        )
        ctx.faces_output = wp.zeros(
            ctx.dim,
            dtype=wp.int32,
            device=wp.device_from_torch(points.device),
            requires_grad=False,
        ...
        return wp.to_torch(
            ctx.distances_output, requires_grad=False
        ).detach(), wp.to_torch(ctx.faces_output, requires_grad=False).detach()
...
def point_to_mesh_distance_and_face(
    points, vertices, faces, max_dist, safe_length: bool = False
):
    return PointToMeshDistanceAndFace.apply(
        points.contiguous(), vertices.contiguous(), faces, max_dist, safe_length
    )
...
class PointToMeshDistanceAndFaceUVs(torch.autograd.Function):
    ...
    def forward(ctx, points, vertices, faces, max_dist, safe_length: bool = False):
        ...
        return (
            wp.to_torch(ctx.distances_output, requires_grad=False).detach(),
    ...
    def backward(ctx, grad_distances, dummy_grad_faces, grad_uvs):
        ...
        wp.launch(
            ...
            adj_inputs=[None, ctx.points.grad, None],
            adj_outputs=[
                ctx.distances_output.grad,
                ctx.faces_output.grad,
                ctx.uvs_output.grad,
            ],
            adjoint=True,
            device=ctx.warp_device,
...
def point_to_mesh_distance_and_face_uvs(
    points, vertices, faces, max_dist
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    return PointToMeshDistanceAndFaceUVs.apply(
        points.contiguous(), vertices.contiguous(), faces, max_dist
    )
