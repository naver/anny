...
def vertices_backward_pass(
    model, pose_parameters, phenotype_kwargs, local_changes_kwargs
):
    output = model(
        pose_parameters=pose_parameters,
        phenotype_kwargs=phenotype_kwargs,
...
def main(
    skinning_method: SkinningMethod = "warp_lbs",
    rig: Rig = "anny",
    topology: str = "anny",
    ...
    for batch_size in forward_batch_sizes:
        pose_parameters = roma.Rigid.identity(
            dim=3,
            batch_shape=(batch_size, model.bone_count),
            dtype=dtype,
            device=device,
        ).to_homogeneous()
        phenotype_kwargs = torch.full(
            (batch_size, len(model.phenotype_labels)), 0.5, dtype=dtype, device=device
        )
        local_changes_kwargs = {
            key: torch.zeros((batch_size), dtype=dtype, device=device)
    ...
    for batch_size in backward_batch_sizes:
        ...
        local_changes_kwargs = {
            key: torch.zeros((batch_size), dtype=dtype, device=device).requires_grad_(
                True
            )
            for key in model.local_change_labels
        }

        vertices_backward_pass(
            model, pose_parameters, phenotype_kwargs, local_changes_kwargs
        )  # Warm-up
