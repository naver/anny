...
class TestSomaRigVertexCount(unittest.TestCase):
    def test_soma_rig_soma_topology_vertex_count_matches_anny_rig(self):
        ...
        self.assertEqual(
            soma_model.template_vertices.shape, anny_model.template_vertices.shape
        )
...
class TestSomaRigProcrustesOrientation(unittest.TestCase):
        ...
        self.assertLess(
            torch.max(
                roma.rotmat_geodesic_distance(
                    ...
                    rest_bone_poses[0, refiner.leaf_bone_parent_indices, :3, :3],
                )
            ),
            1e-9,
        )
    ...
    def test_skeleton_fit_parity_with_soma_package(self):
        ...
        def align_vectors_no_skinning_vnorm(A, B, eps=1e-8, method="kabsch"):
            ...
            return (
                newton_schulz(H, num_iters=20, eps=eps)
        ...
        with mock.patch(
            "soma.geometry.skeleton_transfer.align_vectors",
        ...
        geodesic = roma.rotmat_geodesic_distance(
            fitted[:, 1:, :3, :3],
            (P[None, None] @ rest["rest_bone_poses"][:, :, :3, :3])[:, 1:],
        )
    ...
    def test_orientation_topology_independence(self):
        ...
        blendshape_coeffs = torch.rand(
            (4, soma_topology_model.blendshapes.shape[0]),
            dtype=soma_topology_model.dtype,
        )
        self.assertLess(
            self._max_rest_pose_difference(
                soma_topology_model, anny_topology_model, blendshape_coeffs
            ),
            1e-6,
        )
    ...
    def test_blendshape_labels(self):
        for model in [
            anny.Anny(rig="soma", topology="soma"),
            anny.Anny(rig="anny", topology="anny"),
        ]:
            self.assertEqual(len(model.blendshape_labels), model.blendshapes.shape[0])
            self.assertEqual(
                len(set(model.blendshape_labels)), len(model.blendshape_labels)
            )
    ...
    def test_orientation_local_changes_slicing(self):
        for facial_actions in [False, True]:
            ...
            all_index = {
                label: i for i, label in enumerate(all_model.blendshape_labels)
            }
            arbitrary_subset = [
                all_model.local_change_labels[0],
                all_model.local_change_labels[-1],
            ]
            for local_changes in ["default", arbitrary_subset]:
                ...
                torch.manual_seed(0)
                subset_coeffs = torch.rand(
                    (4, subset_model.blendshapes.shape[0]), dtype=subset_model.dtype
                )
                all_coeffs = torch.zeros(
                    (4, all_model.blendshapes.shape[0]), dtype=all_model.dtype
                )
                for j, label in enumerate(subset_model.blendshape_labels):
                    all_coeffs[:, all_index[label]] = subset_coeffs[:, j]
                poses_subset = subset_model.get_rest_model(subset_coeffs)[
                    "rest_bone_poses"
                ]
...
class TestSomaForwardParity(unittest.TestCase):
    ...
    def test_faces_match_soma_package(self):
        ...
        self.assertTrue(
            torch.equal(
                anny_soma.faces.to(torch.int64), soma_layer.faces.to(torch.int64)
            )
        )
    ...
    def test_forward_vertices_match_soma_package(self):
        ...
        pose_parameters = roma.Rigid(
            roma.rotvec_to_rotmat(extended_rotvec), translation=None
        ).to_homogeneous()
        ...
        error = torch.linalg.norm(
            anny_output["vertices"] - soma_output["vertices"], dim=-1
        )
