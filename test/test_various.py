...
class TestVarious(unittest.TestCase):
    ...
    def test_batch_consistency(self):
        ...
        for k in model.bone_labels:
            ...
            joints_relative_transforms[k] = roma.Rigid(
                rot, torch.zeros((batch_size, 3), dtype=self.dtype, device=self.device)
            ).to_homogeneous()
        ...
        phenotype_kwargs = {
            key: torch.rand(
                (batch_size,), dtype=self.dtype, device=self.device, generator=generator
            )
        ...
        for skinning_method in skinning_methods:
            ...
            # Run the model
            batched_results = model(
                phenotype_kwargs=phenotype_kwargs, pose_parameters=delta_transforms
            )
            ...
            for i in range(batch_size):
                results = model(
                    phenotype_kwargs={
                        key: value[None, i] for key, value in phenotype_kwargs.items()
                    },
                    pose_parameters=delta_transforms[None, i],
                )
                for key in batched_results.keys():
                    self.assertTrue(
                        torch.all(
                            torch.abs(
                                batched_results[key][i] - results[key].squeeze(dim=0)
                            )
                            < epsilon
                        )
                    )
    ...
    def test_single_phenotype_batch_expands_to_pose_batch_size(self):
        ...
        phenotype_kwargs = {
            key: torch.full((1,), 0.5, dtype=self.dtype, device=self.device)
            for key in model.phenotype_labels
        }

        results = model(
            phenotype_kwargs=phenotype_kwargs, pose_parameters=pose_parameters
        )
    ...
    def test_forward_dict_and_tensor_parameters_match(self):
        ...
        local_changes = torch.rand(
            (2, len(model.local_change_labels)), dtype=self.dtype
        )
        phenotype_dict = dict(zip(model.phenotype_labels, phenotype.unbind(dim=1)))
        local_changes_dict = dict(
            zip(model.local_change_labels, local_changes.unbind(dim=1))
        )
        ...
        torch.testing.assert_close(
            tensor_output["bone_poses"], dict_output["bone_poses"]
        )
    ...
    def test_local_changes(self):
        ...
        phenotype_kwargs = dict(
            ...
            caucasian=torch.rand(
                (batch_size,), dtype=self.dtype, device=self.device, generator=generator
            ),
        )

        blendshape_coeffs0 = model.get_phenotype_blendshape_coefficients(
            **phenotype_kwargs
        )
        rest_model0 = model.get_rest_model(blendshape_coeffs0)

        blendshape_coeffs1 = model_local_changes.get_phenotype_blendshape_coefficients(
            **phenotype_kwargs
        )
        rest_model1 = model_local_changes.get_rest_model(blendshape_coeffs1)

        _, phenotype_parameters, _, _ = model_local_changes.get_tensor_inputs(
            phenotype_kwargs=phenotype_kwargs,
            pose_parameters=None,
            local_changes_kwargs=None,
            facial_actions=None,
        )
        local_changes = torch.zeros(
            (batch_size, len(model_local_changes.local_change_labels)),
            dtype=self.dtype,
            device=self.device,
        )
        facial_actions = torch.zeros(
            (batch_size, len(model_local_changes.facial_action_labels)),
            dtype=self.dtype,
            device=self.device,
        )

        blendshape_coeffs2 = model_local_changes._get_phenotype_blendshape_coefficients(
            phenotype_parameters, local_changes, facial_actions
        )
        rest_model2 = model_local_changes.get_rest_model(blendshape_coeffs2)

        for key in ["rest_vertices", "rest_bone_poses"]:
            self.assertTrue(
                torch.all(torch.abs(rest_model1[key] - rest_model0[key]) < 1e-3)
            )
            self.assertTrue(
                torch.all(torch.abs(rest_model2[key] - rest_model0[key]) < 1e-3)
            )
