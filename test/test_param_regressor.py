...
class TestUtils(unittest.TestCase):
    def test_fixed_shape(self):
        ...
        shape_dist = SimpleShapeDistribution(
            ...
            morphological_age_distribution=torch.distributions.Uniform(
                low=torch.tensor(20.0, dtype=model.dtype, device=model.device),
        ...
        with torch.no_grad():
            output = model(
                pose_parameters=pose_parameters, phenotype_kwargs=phenotype_kwargs
            )
        ...
        self.assertTrue(
            pve < epsilon, f"Reconstruction error is too high: {1000.0 * pve:.4f} mm"
        )
    ...
    def test_optimize_shape_except_age_gender(self):
        ...
        initial_phenotype_kwargs = {
            k: v for k, v in phenotype_kwargs.items() if k not in ["age", "gender"]
        }
        initial_phenotype_kwargs["age"] = 0.7 * torch.ones(
            batch_size, dtype=dtype, device=device
        )  # fixed age (adult)
        initial_phenotype_kwargs["gender"] = 0.5 * torch.ones(
            batch_size, dtype=dtype, device=device
        )  # fixed gender (neutral)
        pose, macro, vertices_hat = fitter(
            vertices_target=vertices_target,
        ...
        epsilon = 1e-2  # 10 mm
        self.assertTrue(
            pve < epsilon, f"Reconstruction error too high: {1000.0 * pve:.4f} mm"
        )

        # check age and gender stayed unchanged
        if "age" in macro:
            self.assertTrue(
                torch.allclose(macro["age"], initial_phenotype_kwargs["age"], atol=1e-5)
            )

        if "gender" in macro:
            self.assertTrue(
                torch.allclose(
                    macro["gender"], initial_phenotype_kwargs["gender"], atol=1e-5
                )
            )
    ...
    def test_fit_smplx_templates(self):
        ...
        for obj_path in obj_files:
            ...
            vertices_target = torch.tensor(mesh.vertices, dtype=dtype, device=device)[
                None
            ]  # batch=1

            # initialize phenotype parameters to neutral values
            initial_phenotype_kwargs = {
                key: 0.5 * torch.ones(1, dtype=dtype, device=device)
                for key in model.phenotype_labels
            }
            if "female" in str(obj_path):
                initial_phenotype_kwargs["gender"] = 1.0 * torch.ones(
                    1, dtype=dtype, device=device
                )
            elif "male" in str(obj_path):
                initial_phenotype_kwargs["gender"] = 0.0 * torch.ones(
                    1, dtype=dtype, device=device
                )
            else:
                initial_phenotype_kwargs["gender"] = 0.5 * torch.ones(
            ...
            morpho_age = 20
            initial_phenotype_kwargs["age"] = age_mapping.morphological_to_anny_age(
                torch.Tensor([morpho_age])
            ...
            print(f"{obj_path} → mean PVE: {1000.0 * pve:.4f} mm")

            epsilon = 0.015  # 15 mm tolerance
            self.assertTrue(
                pve < epsilon,
                f"Reconstruction error too high for {obj_path}: {1000.0 * pve:.4f} mm",
            )
        ...
        for topology in ["smplx", "smpl"]:
            ...
            try:
                if topology == "smplx":
                    ...
                    bone_count = 23
                    model2 = SMPL(topology=topology, model_path=model_path).to(
                ...
                self.skipTest(
                    f"Skipping SMPL-X fitting test: SMPLX instantiation failed: {e}"
                )
            ...
            body_pose = 0.2 * torch.randn(
                (batch_size, bone_count * 3), dtype=dtype, device=device
            )
            ...
            left_hand_pose = 0.2 * torch.randn(
                (batch_size, 15 * 3), dtype=dtype, device=device
            )
            right_hand_pose = 0.2 * torch.randn(
                (batch_size, 15 * 3), dtype=dtype, device=device
            )
            ...
            vertices_target = output[
                "vertices"
            ]  # torch.Tensor of shape [batch_size, 13784, 3]
            # --------------------------------------------------------------------#

            # Instantate the Anny body model and the fitter
            rig = "anny-noeyes-notoes-nohands-nobreasts"
            # rig = 'soma'
            model1 = anny.Anny(rig=rig, topology=topology).to(
                dtype=dtype, device=device
            )
            fitter = anny.ParametersRegressor(model=model1, verbose=True, max_n_iters=5)

            # Run the fitting procedure
            pose, phenotype, vertices_hat = fitter(
                vertices_target=vertices_target,
                optimize_phenotypes=True,
                excluded_phenotypes=["age", "gender"],
            )
            ...
            # Compute reconstruction error
            pve = torch.norm(vertices_hat - vertices_target, dim=-1).mean()  # in mm
            print(f"Mean PVE: {1000.0 * pve:.4f} mm")
