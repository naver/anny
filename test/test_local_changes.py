...
class TestLocalChanges(unittest.TestCase):
    def test_local_changes(self):
        ...
        batch_size = 10
        phenotype_kwargs = {
            k: torch.rand(batch_size, dtype=dtype)
            for k in withoutlocal.phenotype_labels
        }
        ...
        for label in ["rest_vertices", "vertices", "bone_poses"]:
            self.assertTrue(
                torch.all(
                    torch.abs(output_withoutlocal[label] - output_withlocal[label])
                    < 1e-8
                ),
                f"Outputs differ for label {label} between with and without local changes",
            )
            self.assertTrue(
                torch.all(
                    torch.abs(output_withoutlocal[label] - output_withsomelocal[label])
                    < 1e-8
                ),
        ...
        local_changes_kwargs = {
            k: torch.randn(batch_size, dtype=dtype) for k in local_change_labels
        }
        output_withlocal = withlocal(
            phenotype_kwargs=phenotype_kwargs, local_changes_kwargs=local_changes_kwargs
        )
        output_withsomelocal = withsomelocal(
            phenotype_kwargs=phenotype_kwargs, local_changes_kwargs=local_changes_kwargs
        )
        ...
        for label in ["rest_vertices", "vertices", "bone_poses"]:
            self.assertTrue(
                torch.all(
                    torch.abs(output_withlocal[label] - output_withsomelocal[label])
                    < 1e-8
                ),
                f"Outputs differ for label {label} between with and with some local changes",
            )
