...
class TestPoseTransfer(unittest.TestCase):
    def test_transfer_reproduces_pose_across_rest_orientations(self):
        ...
        src_model = anny.Anny(
            rig="makehuman", topology="anny", local_changes="default"
        ).to(dtype=dtype)
        target_model = anny.Anny(
            rig="anny", topology="anny", local_changes="default"
        ).to(dtype=dtype)

        torch.manual_seed(0)
        phenotype_kwargs = {
            label: torch.rand((), dtype=dtype)
            for label in target_model.phenotype_labels
        }
        local_changes_kwargs = {
            label: 2 * torch.rand((), dtype=dtype) - 1
            for label in target_model.local_change_labels
        }
        ...
        rotvecs[:, shared_indices] = 0.3 * torch.randn(
            (1, len(shared_indices), 3), dtype=dtype
        )
        ...
        translations[:, shared_indices] = 0.05 * torch.randn(
            (1, len(shared_indices), 3), dtype=dtype
        )
        src_pose_parameters = roma.Rigid(
            roma.rotvec_to_rotmat(rotvecs), translations
        ).to_homogeneous()
